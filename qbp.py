...
#
# 16-Nov-2025: Initial GPU support in insideout_DL(). This is done
#              by turning insideout_DL into a wrapper function to either
#              insideout_DL_cuda or insideout_DL_np. The former uses
#              pytorch to access cuda GPU. The backend is decided by
#              the global parameter INSIDEOUT_DL_BACKEND, which can be
#              one of 'NP', 'NP-SP', 'CUDA', 'CUDA-SP', where '-SP'
...
#
# 'NP'         --- numpy (CPU)
# 'NP-SP'      --- numpy (CPU), single-precision
# 'CUDA',      --- pytorch-CUDA (GPU)
# 'CUDA-SP'    --- pytorch-CUDA (GPU), single-precision
#
# When non '-SP' option is chosen, the calculation is done in the
# precision of the input tensors (which can be either double- or -single
...
if INSIDEOUT_DL_BACKEND in ['CUDA', 'CUDA-SP']:
	try:
		...
		print("qbp error: cannot use INSIDEOUT_DL_BACKEND " \
			f"= '{INSIDEOUT_DL_BACKEND}' because torch is not installed")
		exit(1)

	#
	# OK, so we have pytorch. Let's see if we have a CUDA GPU device
	#
	...
	if torch.cuda.is_available():
		...
		print("\n\n")
		print("                     * * *    qbp.py  WARNNING    * * * \n")
		print("Cannot use INSIDEOUT_DL_BACKEND " \
			f"= '{INSIDEOUT_DL_BACKEND}' because no CUDA device is found.\n")
		print("                Using the pytorch CPU backend instead")
		print("                -------------------------------------\n\n\n")


		CUDA_GPU = torch.device("cpu")  # this is just for debugging.
...
def insideout_DL_cuda(T1, in_m_list1):

	r"""

	A DOUBLE-LAYER insideout using pytorch cuda via the tensordot
	function.

	Given a ket tensor T and a list of incoming messages, calculate the
	outgoing BP messages.

	The function first moves the np arrays to pytorch tensors (at the
	appropriate precision), and then moves them to the cuda GPU, where
	the calculation is done.

	Input Parameters:
	-------------------

	T1         --- The ket tensors. Should be of the form [d, D0, D1, D2, ...]

	in_m_list1 --- The incoming messages. If T has k logical legs, then
	...
	legs_no = len(T1.shape)-1

	#
	# First, we move the arrays to CUDA GPU
	#

	T = torch.from_numpy(T1).to(CUDA_GPU)
	in_m_list = [torch.from_numpy(m).to(CUDA_GPU) for m in in_m_list1]

	#
	# Now we make sure all of them are of the same type because pytorch
	# tensordot does not know how to mix float and complex tensors
	#

	T_list = in_m_list + [T]
	maxty = torch.float32    # The "common denominator" type to be found

	for t in T_list:

		if t.dtype==torch.complex128:
			maxty = torch.complex128
			break

		elif t.dtype==torch.complex64:
			if maxty in [torch.float32, torch.float64]:
				maxty = torch.complex64

		elif t.dtype==torch.float64:
			if maxty == torch.float32:
				maxty = torch.float64

	T = T.type(maxty)
	for i in range(len(in_m_list)):
		in_m_list[i] = in_m_list[i].type(maxty)
	...
	for leg in range(legs_no-1):
		...
		ketT = torch.tensordot(ketT, in_m_list[legs_no-1-leg], dims=([legs_no-leg],[0]))
		ket_Ts.append(ketT)
	...
	for i in range(legs_no):
		...
		out_m = torch.tensordot(ket_Ts[legs_no-i-1], braT, dims=(ket_axes, bra_axes))
		...
		if i<legs_no-1:
			...
			# its first leg
			#
			braT = torch.tensordot(braT, in_m_list[i], dims=([1],[1]))

		#
		# Make the outgoing message explicitly hermitian
		#
		out_m = 0.5*(out_m + torch.conj(out_m.T))

		out_m_list.append(out_m)


	#
	# Now move back to numpy CPU
	#

	out_m_list1 = [m.cpu().numpy() for m in out_m_list]
...
def dp_to_sp(a):
	...
	Reduce the precision of a float64 or complex128 numpy array to
	a single precision float32 or complex64

	"""

	if not a.dtype in [np.float32, np.float64, np.complex64, np.complex128]:
		print(f"dp_to_sp() error: cannot convert type {a.dtype} "\
			f"to single-precision")
		exit(1)
...
def insideout_DL(T, in_m_list):
	...
	the numpy backend or the pytorch-CUDA backend.

	"""
	#
	# First check if we need to move to single precision
	#
	...
	#
	# Now see if we call the CUDA backend or the NP backend
	#
